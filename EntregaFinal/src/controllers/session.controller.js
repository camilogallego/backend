import { UserService, RestorePasswordRequestService } from "../services/index.js";
import { CurrentUserDTO } from "../dto/index.js";
import { Logger, ROLES } from "../helpers/index.js";
import { createHash, arePasswordsEqual } from "../utils.js";
  ...
  current = async (req, res) => {
    if (!req.user) {
      return res.status(400).send({
        status: "error",
        error: "You need to be logged in to see this",
      });
    }
    ...
    try {
      ...
      if (user && user._id) {
        const restorePasswordRequest =
          await RestorePasswordRequestService.getRestorePasswordRequest(
            user._id
          );
        if (
          restorePasswordRequest &&
          restorePasswordRequest.expiresAt > Date.now()
        ) {
        ...
        await RestorePasswordRequestService.sendRestorePasswordMail(
          email,
          user._id
        );
