...
class UserController {
  ...
  uploadDocuments = async (req, res) => {
    ...
    const profiles = req.files.profile
      ? req.files.profile.map((file) => ({
          name: file.originalname,
          reference: file.path,
        }))
      : [];
    const products = req.files.product
      ? req.files.product.map((file) => ({
          name: file.originalname,
          reference: file.path,
        }))
      : [];
    ...
    const updateResult = await UserService.updateDocuments(
      uid,
      updatedDocuments
    );
    ...
    if (!updateResult) {
      return res
        .status(404)
        .json({ error: "No se pudieron subir los documentos" });
    }
  ...
  updateRole = async (req, res) => {
    ...
    if (req.user.role !== ROLES.ADMIN) {
      return res
        ...
        .send({
          status: "error",
          error: "You don't have permissions to change user role",
        });
    }
    const { uid } = req.params;
    ...
    try {
      ...
      if (req.user._id === uid) {
        ...
        return res.status(400).json({
          ok: false,
          message:
            "Usuarios no eliminados. No puede eliminar su propio usuario",
        });
      }
      const user = await UserService.getUserById(uid);
