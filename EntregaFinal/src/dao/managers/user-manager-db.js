...
const sendDeleteInactiveUserMail = async (email, username, lastConnection) => {
...
class UserManager {
  getAllUsers = async () => {
    try {
      ...
      CustomError.createError(
        ERRORS.DATABASE_ERROR.name,
        "",
        "Can not get all users",
  ...
  getUser = async (email = "") => {
    ...
    try {
      ...
      CustomError.createError(
        ERRORS.INVALID_PARAMETER_ERROR.name,
        "",
        "Can not get user with the provided email ",
        ERRORS.INVALID_PARAMETER_ERROR.code
      );
    }
  };
  ...
  getUserById = async (id = "") => {
    ...
    try {
      ...
      CustomError.createError(
        ERRORS.INVALID_PARAMETER_ERROR.name,
        "",
        "Can not get user with the provided id ",
        ERRORS.INVALID_PARAMETER_ERROR.code
      );
    }
  };
  ...
  addUser = async ({
    ...
    try {
      ...
      Logger.error(
        "🚀 ~ file: User.manager.js:21 ~ UserManager ~ addUser=async ~ error:",
        error
      );

      CustomError.createError(
        ERRORS.CREATION_ERROR.name,
        "",
        "Can not create new user",
        ERRORS.CREATION_ERROR.code
      );
    }
  };
  ...
  updateLastLogIn = async (userId) => {
    try {
      ...
      Logger.error(
        "🚀 ~ file: User.manager.js:21 ~ UserManager ~ updateLastLogIn=async ~ error:",
        error
      );
      CustomError.createError(
        ERRORS.CREATION_ERROR.name,
        "",
        "Can not set last connection",
        ERRORS.CREATION_ERROR.code
      );
    }
  };
  ...
  updateDocuments = async (userId, docs) => {
    try {
      ...
      const uploadedDocuments = docs.filter((doc) =>
        doc.reference.includes("\\documents\\")
      );
      const uploadedDocumentNames = uploadedDocuments.map((doc) => doc.name);
      const allRequiredDocumentsUploaded = requiredDocumentsNames.every(
        (requiredName) =>
          uploadedDocumentNames.some((uploadedName) =>
  ...
  deleteInactiveUsers = async () => {
    try {
      ...
      for (const user of users) {
        await sendDeleteInactiveUserMail(
          user.email,
          user.email,
